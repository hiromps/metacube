...
export default function SubscriptionPlansCard({ onSelectPlan }: SubscriptionPlansCardProps) {
  ...
  const getPlanPrice = (plan: Plan) => {
    if (plan.id === 'pro' && isYearly && plan.yearlyPrice) {
  ...
  const getPlanOriginalPrice = (plan: Plan) => {
    if (plan.id === 'pro' && isYearly && plan.yearlyOriginalPrice) {
      return plan.yearlyOriginalPrice
    }
    return plan.originalPrice
  }

  const getPlanBillingPeriod = (plan: Plan) => {
    if (plan.id === 'pro' && isYearly && plan.yearlyPrice) {
      return '/ 年'
    }
    return '/ 月'
              ...
              <div className="text-center mb-4">
                ...
                {plan.id === 'pro' && (
                  <div className="flex justify-center mb-3">
                    <div className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-full p-0.5 flex items-center">
                      <button
                        onClick={(e) => {
                          e.stopPropagation()
                          setIsYearly(false)
                      ...
                      >
                        年額
                        <span className="ml-1 text-xs bg-yellow-500/20 text-yellow-300 px-1 py-0.5 rounded-full">
                          お得
                        </span>
                      </button>
                    </div>
                ...
                <div className="flex items-center justify-center gap-2">
                  ...
                  {getPlanOriginalPrice(plan) && (
                    <span className="text-sm text-white/50 line-through">
                      ¥{getPlanOriginalPrice(plan)!.toLocaleString()}
                    </span>
                  )}
                </div>
                <p className="text-xs text-white/60">
                  {getPlanBillingPeriod(plan)}
                  {plan.id === 'pro' && isYearly && plan.yearlyPrice && (
