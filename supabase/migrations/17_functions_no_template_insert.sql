-- Create essential functions without template INSERT
-- Since template already exists, we only need the functions
...
-- 2. Complete .ate generation (uses existing template)
...
BEGIN
    ...
    -- Get existing template by name (since smartgram template exists)
    SELECT * INTO template_record
    FROM ate_templates
    WHERE name = queue_record.template_name
    LIMIT 1;

    IF NOT FOUND THEN
        -- If template not found, use the existing smartgram template
        SELECT * INTO template_record
        FROM ate_templates
        WHERE name = 'smartgram'
        LIMIT 1;
    END IF;
