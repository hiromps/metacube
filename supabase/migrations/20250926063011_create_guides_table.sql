...
CREATE TABLE IF NOT EXISTS guides (
  ...
  CONSTRAINT guides_order_index_unique UNIQUE(order_index)
);

-- インデックス作成
CREATE INDEX guides_is_active_idx ON guides(is_active);
CREATE INDEX guides_category_idx ON guides(category);
CREATE INDEX guides_order_idx ON guides(order_index);
...
-- サンプルデータを挿入（オプション）
INSERT INTO guides (title, description, category, order_index, content) VALUES
