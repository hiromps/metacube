...
function App:init()
    ...
    -- 開始通知（デバッグモードの時のみ）
    if Config.debug then
        toast("🚀 自動いいね開始！目標: " .. Config.maxLikeCount .. "回")
    end
end
...
function App:createUsageLog(likeCount, duration)
    -- 複数のパスを順番に試行
    local pathsToTry = {
        "usage_log.txt",                                          -- 1. 相対パス（最も確実）
        "/var/mobile/Library/AutoTouch/Scripts/usage_log.txt",   -- 2. Scripts フォルダ
        "/var/mobile/Library/AutoTouch/usage_log.txt",           -- 3. AutoTouch フォルダ
        "/tmp/usage_log.txt"                                     -- 4. 最後の手段
    }
    ...
    for i, path in ipairs(pathsToTry) do
        local success, fileHandle = pcall(io.open, path, "a")
    ...
    if not file then
        if Config.debug then
            print("❌ 全てのパスで使用量ログファイル作成失敗")
        end
        return false
    end

    local timestamp = os.date("%Y-%m-%d %H:%M:%S")
    local scriptName = "timeline.lua"
    local logEntry = string.format(
        "%s | %s | 実行時間: %.2f秒 | いいね数: %d回 | 成功\n",
        timestamp,
        scriptName,
        duration,
        likeCount
...
function App:run()
    ...
    -- 完了通知（常に表示）
    toast("🎊 自動いいね完了！総数: " .. self.likeCount .. "回")
    ...
    -- 使用量ログファイルの作成
    local success = self:createUsageLog(self.likeCount, duration)
    if success then
        if Config.debug then
            print("📊 スクリプト実行成功")
            print("📊 使用量ログ記録: timeline.lua")
        end
    else
        if Config.debug then
            print("⚠️ 使用量ログファイル作成失敗")
        end
    end
...
if settings then
    ...
    -- 確認メッセージ（常に表示）
    toast("設定完了！" .. Config.maxLikeCount .. "回のいいねを実行します")
    ...
    if success then
        ...
        if runSuccess then
            -- 正常終了
            log("✅ timeline.lua を正常に実行しました")
        elseif tostring(runErr):match("interrupted") then
            toast("⚠️ ユーザーによって中断されました")
