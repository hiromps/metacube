#!/usr/bin/env python3
"""
Simple AutoTouch .ate file structure analyzer without unicode
"""
...
def analyze_zip_structure(filename):
    ...
    print(f"[ANALYSIS] Analyzing AutoTouch file: {filename}")
    print("=" * 60)
    ...
    while offset < len(data):
        ...
        if signature == 0x04034b50:  # Local file header
            print(f"[LOCAL_HEADER] Entry #{entry_count + 1} at offset {offset:04X}")
            ...
            if extra_len > 0 and extra_offset + extra_len <= len(data):
                ...
                if len(extra_data) >= 11:
                    if extra_data[0:2] == b'\x01\x99':  # AES extra field ID
                        print(f"   [AES] AES Extra Field Found:")
            ...
            if comp_size > 0 and data_offset + comp_size <= len(data):
                ...
                print(f"   [DATA] Encrypted data: {len(encrypted_data)} bytes")
                ...
                if len(encrypted_data) >= 26:  # 16 bytes salt + 10 bytes auth code minimum
                    ...
                    print(f"   [SALT] Probable salt (16 bytes): {salt.hex()}")
                    print(f"   [CONTENT] Encrypted content: {len(content)} bytes")
                    print(f"   [AUTH] Probable auth code (10 bytes): {auth_code.hex()}")
            ...
            print(f"[CENTRAL_DIR] Central directory header at offset {offset:04X}")
            ...
            print(f"[END_DIR] End of central directory at offset {offset:04X}")
            ...
            print(f"[UNKNOWN] Unknown signature: 0x{signature:08X} at offset {offset:04X}")
            break

    print(f"\n[RESULT] Analysis complete. Found {entry_count} file entries.")
